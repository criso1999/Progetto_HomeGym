...
<%
    // convenience: recupera il post come org.bson.Document (fornito dal servlet)
    Object maybe = request.getAttribute("post");
    Document post = null;
    if (maybe instanceof Document) post = (Document) maybe;
%>
...
<body>
  ...
  <c:if test="${empty post}">
    ...
    <c:remove var="post" scope="request"/>
    <c:if test="${false}"></c:if> <!-- no-op to keep JSP analyzer calm -->
    <%
      return;
    %>
  </c:if>
  ...
  <div class="post">
    ...
    <p>
      ...
      <c:if test="${not empty sessionScope.user}">
        &nbsp;|&nbsp;<a href="${pageContext.request.contextPath}/posts/view?id=${post._id}#comments">Jump to comments</a>
      </c:if>
    </p>
  </div>
  ...
  <div class="eval">
    ...
    <!-- show evaluation form only to trainers (PERSONALE/PROPRIETARIO) -->
